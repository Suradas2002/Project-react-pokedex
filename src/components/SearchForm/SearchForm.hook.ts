...
import { pokemonListService,pokemonDetailServices } from "../../service";
import { useForm } from "react-hook-form"
import { usePokemonListStore } from "../../store/pokemonList";
import { generationList } from "../../untill/optionList";
...
const useSearchForm = () =>{
    ...
    const filterPokemon = (pokeList:IPokemonDetailResponse[], keyword:string,type:string,sort:'id'|'name')=>{
        ...
        const typeFilter = type !== 'all types' ? keywordFilter.filter((item)=>item.types.find((f)=>f.type.name.toLocaleLowerCase().includes(type.toLocaleLowerCase()))
        ):keywordFilter
        return sortBy(typeFilter,sort)
    }

    const sortBy = (data:IPokemonDetailResponse[], type:'id'|'name')=>{ switch (type) {
        case 'id':
